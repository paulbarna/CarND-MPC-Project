...
// Tunable parameters vector
Eigen::VectorXd TunableParams(7);
...
public:
	...
	void operator()(ADvector& fg, const ADvector& vars) {
		...
		for (int t = 0; t < N; t++) {
		  fg[0] += TunableParams[0]*CppAD::pow(vars[cte_start + t] , 2);
		  fg[0] += TunableParams[1]*CppAD::pow(vars[epsi_start + t], 2);
		  fg[0] += TunableParams[2]*CppAD::pow(vars[v_start + t]-vRef, 2);
		}

		// Minimize change-rate.
		for (int t = 0; t < N - 1; t++) {
		  fg[0] += TunableParams[3]*CppAD::pow(vars[delta_start + t], 2);
		  fg[0] += TunableParams[4]*CppAD::pow(vars[a_start + t], 2);
		}

		// Minimize the value gap between sequential actuations.
		for (int t = 0; t < N - 2; t++) {
		  fg[0] += TunableParams[5]*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
		  fg[0] += TunableParams[6]*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
		}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	//set the range of values δ\deltaδ to [-25, 25] in radians
	...
	output_values.push_back(solution.x[delta_start]);
	output_values.push_back(solution.x[a_start]);

    for (int i = 0; i < N-1; i++)
    {
    	output_values.push_back(solution.x[x_start+i+1]);
    	output_values.push_back(solution.x[y_start+i+1]);
    }
